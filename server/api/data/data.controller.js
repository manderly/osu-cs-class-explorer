...
function buildCourseData() {
  ...
  .then(rawData => {
    ...
    underscore.each(rawData, course => {
      ...
      key = course['whatCourseDidYouTake?'].substring(0, 6).split(' ')
        .join('');
  ...
  .catch(() =>
    'Error building spreadsheet data - could be a problem with Google Spreadsheet, your Google API credentials, or the packages used to access it.'
  );
}
